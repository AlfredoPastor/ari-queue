...
func (ag *AgentRepositoryMock) Upsert(ctx context.Context, agent domain.Agent) error {
...
func (ag *AgentRepositoryMock) Search(ctx context.Context, id uuid.VoId) (domain.Agent, error) {
	ret := ag.Called(ctx, id)

	var r0 domain.Agent
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, uuid.VoId) (domain.Agent, error)); ok {
		r0, r1 = rf(ctx, id)
	} else {
		r1 = ret.Error(1)
	}
